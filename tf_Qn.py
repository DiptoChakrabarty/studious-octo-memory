...
class DeepQNetwork(object):
    def __init__(self,lr,n_actions,name,fcl_dims=256,input_dims=(210,210,4),chkt_dir='.models/dqn'):
    # learning rate , model name, dimensions of model,input dimensiosn, directory
        self.lr=lr 
        self.name=name 
        ...
        self.build_network()    # Build everything for the graph
        self.sess.run(tf.global_variables_initializer) 
        self.saver= tf.train.Saver() # Save model
        self.checkpoint_file = os.path.join(chkt_dir,'deepnet.ckpt')
    ...
    def build_net(self):
        with tf.variable_scope(self.name):
            ...
            kernel_initializer=tf.variance_scaling_initializer(scale=2))  # Make Convulation layer
            conv1_activated = tf.nn.relu(conv1)  # Provide activation function
            ...
            flat= tf.layers.flatten(conv3_activated)   # flattening layer and then the dense layer 
            ...
            # get linear values of the output  and then actual values
            self.q = tf.reduce_sum(tf.multiply(self.Q_values,self.actions))
            self.loss = tf.reduce_mean(tf.square(self.q- self.q_target))
            # check delta of action taken and maxiaml action that could have been taken
...
class Agent(object):
    def __init__(self, alpha, gamma, mem_size, n_actions, epsilon, batch_size,
                 ...
                 q_next_dir='.model/q_next', q_eval_dir='.model/q_eval'):
    # learning rate , memory size , batch size 
        ...
        self.q_next = DeepQNetwork(alpha,n_actions,input_dims=input_dims,
            name='q-next',chkt_dir=q_next_dir)
        self.q_eval = DeepQNetwork(alpha,n_actions,input_dims=input_dims,
            name='q-eval',chkt_dir=q_eval_dir)

    # two networks ready for evaluation and making 
    # Q learning works based on memory , it saves the flag 
    ...
    def store_transition(self,state,action,reward,state_,terminal):
        # Current state , future state , action taken , reward obtained
    ...
    def choose_action(self,state):
        # select epsilon rate and greedy rate
    ...
    def learn(self):
        ...
        if self.mem_cntr > 25000:#200000:
            if self.epsilon > 0.05:
                self.epsilon *= 4e-7   # decrease epsilon after each case
